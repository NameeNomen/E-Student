<x-app-layout>
    ...
    <x-slot name="header">
        {{-- Slot header bawaan dikosongkan --}}
    </x-slot>

    <div class="min-h-screen bg-slate-50 pb-20 font-sans">
        
        {{-- ========================================== --}}
        {{-- 2. HERO SECTION (POLA SIRKUIT GEOMETRIS) --}}
        {{-- ========================================== --}}
        <div class="bg-[#009da5] pb-32 pt-12 rounded-b-[3.5rem] shadow-2xl relative overflow-hidden">
            ...
            <div class="absolute inset-0 z-0 pointer-events-none opacity-20">
                <svg width="100%" height="100%" viewBox="0 0 1440 600" preserveAspectRatio="xMidYMid slice" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    {{-- Dekorasi Dots --}}
                    <circle cx="800" cy="150" r="2" fill="white" fill-opacity="0.6"/>
                    <circle cx="500" cy="100" r="2" fill="white" fill-opacity="0.5"/>
                    <circle cx="1200" cy="400" r="3" fill="white" fill-opacity="0.3"/>
                </svg>
            </div>
                ...
                <div class="flex flex-col md:flex-row items-center justify-between gap-6">
                    ...
                    <div class="w-full md:w-auto min-w-[300px]">
                        <div class="relative group">
                            ...
                            <input type="text" 
                                class="block w-full pl-11 pr-4 py-3.5 bg-white/10 border border-white/20 rounded-2xl leading-5 text-white placeholder-teal-100 focus:outline-none focus:bg-white/20 focus:border-white/40 focus:placeholder-white focus:ring-0 transition-all duration-300 shadow-lg backdrop-blur-sm"
        ...
        {{-- ========================================== --}}
        {{-- 3. MAIN CONTENT (STATISTIK & GRID) --}}
        {{-- ========================================== --}}
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-20 relative z-20">
            
            {{-- Statistik Ringkas (Mengambil Data dari Controller) --}}
            ...
            {{-- Grid Mata Kuliah --}}
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 animate-entry delay-200">
                ...
                @foreach ($subjects as $id => $subject)
                    <div class="bg-white rounded-3xl shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden group hover:-translate-y-1 transition-all duration-300">
                        ...
                        <div class="p-6 cursor-pointer relative" onclick="toggleBooks('{{ $id }}')" id="folder-{{ $id }}">
                            <div class="flex justify-between items-start mb-4">
                                <div class="flex items-center space-x-4">
                                    
                                    {{-- UNIVERSAL ICON FOLDER (SVG) --}}
                                    {{-- Satu icon untuk semua, database tidak perlu simpan path icon --}}
                            ...
                            <div class="mt-2">
                                ...
                                <div class="w-full bg-gray-100 rounded-full h-1.5 overflow-hidden">
                                    <div class="bg-[#009da5] h-1.5 rounded-full transition-all duration-1000 ease-out" style="width: {{ $subject['progress'] }}%"></div>
                                </div>
                            </div>
                        </div>

                        {{-- Isi Materi (Hidden by Default) --}}
                        <div id="{{ $id }}" class="hidden bg-slate-50 border-t border-gray-100">
                            <div class="p-2">
                                @forelse ($subject['materials'] as $material)
                                    @php
                                        // Logic Penentuan Warna Icon berdasarkan Tipe File
                                        $iconType = 'fa-file-alt'; 
                                        $colorClass = 'text-gray-500 bg-gray-100';
                                        
                                        if($material['type'] == 'pdf') { $iconType = 'fa-file-pdf'; $colorClass = 'text-red-500 bg-red-50'; }
                                        elseif($material['type'] == 'video') { $iconType = 'fa-play-circle'; $colorClass = 'text-purple-500 bg-purple-50'; }
                                        elseif($material['type'] == 'doc') { $iconType = 'fa-file-word'; $colorClass = 'text-blue-500 bg-blue-50'; }
                                        elseif($material['type'] == 'ppt') { $iconType = 'fa-file-powerpoint'; $colorClass = 'text-orange-500 bg-orange-50'; }
                                        elseif($material['type'] == 'link') { $iconType = 'fa-link'; $colorClass = 'text-teal-500 bg-teal-50'; }
                                    @endphp
    ...
    @push('scripts')
    <script>
        // Fungsi untuk animasi Buka/Tutup Accordion
        window.toggleBooks = function(id) {
            ...
            if (content.classList.contains('hidden')) {
                ...
                // Ubah Style Tombol (Jadi Teal & Putar)
            ...
            if (url) {
                ...
                setTimeout(() => {
                    ...
                    if (url !== '#') {
                        ...
                        console.log('Ini link placeholder dummy');
                    }
                }, 150);
            }
        }
